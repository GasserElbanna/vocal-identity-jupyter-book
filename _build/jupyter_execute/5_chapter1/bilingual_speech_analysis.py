...
# In this section, we explore the impact of language on voice identity processing.

# ## Literature Review:

# A phenomenon known as *Language-familiarity effect* suggests that listeners tend to identify voices more accurately when the language spoken is their native language compared to other languages ([Goggin et al, 1991](https://link.springer.com/article/10.3758/BF03199567); [Perrachione & Wong, 2007](https://www.cambridge.org/core/journals/applied-psycholinguistics/article/abs/learning-pitch-patterns-in-lexical-identification-by-native-englishspeaking-adults/138B3998B82E13C2F8E51256E64E35A7)). Thus, in this section, we are curious if such phenonmenon could be observed in models. Accordingly, we used a corpus with bilingual speakers. Then, we trained different classifiers on one language and test on the same language and a different one to explore the impact of language-familiarity on identity processing in models. Also, to highlight the models that are invariant to such variation.

# ### Research Questions:
# 1) How do different models perceive changes in language?
# 2) Which models would be invariant to language?
# 3) How could language impact the models ability to predict speaker identity?

# ## Dataset Description:

# For this experiment, we used [SpiCE corpus](https://spice-corpus.readthedocs.io/en/latest/). It is a bilingual speech dataset that comprises 30-minute interviews with 10 bilingual speakers. The speakers speak both English and Cantonese. We preprocessed the data by removing the interviewers voice from the utterances using *Audacity*. Also, we cut all the pauses using *Librosa* python package and setting a threshold of 45dB (i.e. below that is considered silent). All the extracted segment from the latter step were concatenated to form utterances containing only the speaker's voice. Finally, we chunked each speaker's utterance into 3-sec utterances. Accordingly, for each speaker, we have 3-sec utterances in both languages, yielding 5238 utterances total for all speakers.
...
# In this section, we evaluate the predictive power of all models to recognize speakers from two languages. We used different ML classifiers such as Logistic Regression (LR), Random Forest (RF), Support Vector Machine (SVM) and a shallow linear layer. The work flow of this section, for each model, is as follows:
# 1) We split the dataset into English and Cantonese subsets.
# 2) Then, for each speaker in a language subset, we split their utterances into train and test (70% and 30%, respectively).
# 3) The classifier will be trained once on the English train subset and tested on both English and Cantonese test subsets.
# 4) Repeating the previous step but this time the classifier is trained on the Cantonese train subset.
# 5) For each classifier, we implement grid search across its hyperparameters and we report performance for each hyperparameters sequence on the test set
# 6) We used Predefined splits for train and test.
# 7) The pipeline comprises standardizing the representations then passing it to the classifier.
# 8) Consequently, the encoder yields several UAR scores for each classifier, each label and eventually each model. (Reported in boxplots below)
...
palette = {'English':'steelblue', 'Cantonese':'chocolate'}


# ### 3.1. Classifiers trained on English utterances

# In[8]:


#read scores from all models across all classifiers as dataframe
df_english = pd.read_csv(f'../{exp_config.dataset_name}/linear_encoding_all_models_scores_eng_train.csv', index_col=0)
...
graph = sns.barplot(data=df_english, x='Model', y='Score', hue='Label', ax=ax, palette=palette)
...
ax.axhline(y=10, c='red')
ax.legend(bbox_to_anchor=(1, 1), fontsize=40)
plt.tight_layout()
plt.savefig(f'../{exp_config.dataset_name}/linear_encoding_plots/linear_encoding_eng_train_{clf}.png')
plt.savefig(f'../{exp_config.dataset_name}/linear_encoding_plots/linear_encoding_eng_train_{clf}.svg')
...
clf = 'SVM'
fig, ax = plt.subplots(1, 1, figsize=(25, 15))
sns.barplot(data=df_english, x='Model', y='Score', hue='Label', ax=ax, palette=palette)
ax.set_xticklabels(ax.get_xticklabels(), size = 35, rotation=80)
ax.set_yticklabels(ax.get_yticks(), size = 35)
ax.set_ylabel('UAR (%)', fontsize=40)
ax.set_xlabel('Models', fontsize=40)
...
# ### 3.2. Classifiers trained on Cantonese utterances

# In[55]:


#read scores from all models across all classifiers as dataframe
df_can = pd.read_csv(f'../{exp_config.dataset_name}/linear_encoding_all_models_scores_can_train.csv', index_col=0)
...
clf = 'LR'
fig, ax = plt.subplots(1, 1, figsize=(25, 15))
graph = sns.barplot(data=df_can, x='Model', y='Score', hue='Label', ax=ax, palette=palette)
ax.set_xticklabels(ax.get_xticklabels(), size = 35, rotation=80)
ax.set_yticklabels(ax.get_yticks(), size = 35)
ax.set_ylabel('UAR (%)', fontsize=40)
ax.set_xlabel('Models', fontsize=40)
...
ax.legend(bbox_to_anchor=(1, 1), fontsize=40)
plt.tight_layout()
plt.savefig(f'../{exp_config.dataset_name}/linear_encoding_plots/linear_encoding_can_train_{clf}.png')
plt.savefig(f'../{exp_config.dataset_name}/linear_encoding_plots/linear_encoding_can_train_{clf}.svg')
...
# #### 4.1.3. Mapping Language (English/Cantonese)
